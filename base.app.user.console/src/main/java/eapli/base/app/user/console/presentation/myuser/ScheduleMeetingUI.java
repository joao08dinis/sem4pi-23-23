...
public class ScheduleMeetingUI extends AbstractUI {
    ...
    protected boolean doShow() {
        ...
        List<Profile> admins = profileController.getAdminProfiles();
        ...
        for (Profile admin : admins) {
            ...
            System.out.println("Name-" + admin.getUser().name().firstName());
            int input;
            System.out.println("Do you want to add this manager to the meeting?");
            System.out.println("1. ACCEPT");
            System.out.println("2. DECLINE");
            MeetingParticipant meetingParticipant = new MeetingParticipant(null,MeetingParticipantState.PENDING, admin);
            do {
                try {
                    input = Console.readInteger("Option: ");
                    switch (input) {
                        case 1:
                            System.out.println("You accepted the admin.");
                            meetingParticipants.add(meetingParticipant);
                            isValidOption = true;
                            break;
                        case 2:
                            System.out.println("You declined the admin.");
                            isValidOption = true;
                            break;
                        default:
                            System.out.println("Invalid option selected. Please try again.");
                    ...
                    System.out.println("An error occurred on select the Status of the admin: " + e.getMessage());
                }
            }while(!isValidOption);
        }
            ...
            StudentProfile studentProfile = currentUserProfile;

            try {
                addMeeting = this.theController.addMeeting(duration, hours, minutes, title, studentProfile, meetingParticipants, MeetingState.ACCEPTED);
            } catch (Exception e) {
                e.printStackTrace();
